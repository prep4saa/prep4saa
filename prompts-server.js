...
🚫🚫🚫 **시나리오 주제는 서비스와 독립적 (절대 위반 금지)** 🚫🚫🚫
주어진 서비스(\${SERVICE_NAMES})가 무엇이든 — 심지어 S3 Object Lock, AWS Backup, AWS Config가 주어져도 — 시나리오 주제는 컴플라이언스가 아닐 수 있습니다.
**시나리오 주제는 12개 도메인 중에서 독립적으로 선택**하세요:
1. CDN/저지연  2. 실시간 스트리밍/IoT  3. 자동 스케일링  4. 비용 최적화
5. DR/RTO·RPO  6. DB 성능/캐싱  7. 서버리스/이벤트 기반  8. 네트워크/하이브리드
9. 컨테이너  10. 분석/데이터 레이크  11. 보안/위협 탐지  12. 컴플라이언스 ← **10문제 중 1문제 이하만 사용**
주어진 서비스가 시나리오 주제와 안 맞으면 **함정답(오답)으로 활용**. 정답이 다른 서비스여도 됨.

🚫 **절대 금지 표현 (S3 Object Lock/Glacier/Lifecycle 정답 문제 외 사용 금지)** 🚫
"X년 동안 보존", "변경 불가능", "불변하게 보관", "데이터 무결성을 보장", "감사 추적", "규정 준수"
이 표현은 **정답이 S3 Object Lock 또는 S3 Glacier 또는 S3 라이프사이클 정책인 문제에서만** 허용. 다른 모든 문제에서는 절대 사용 금지.

🚫 **"7년" 절대 사용 금지** 🚫
보존 기간이 필요하면: 30일 / 90일 / 6개월 / 1년 / 2년 / 5년 / 10년 중 선택. "7년" 무효 처리.

🔍 **응답 전 자가 점검 필수**:
1. 시나리오에 "보존", "불변", "X년" 표현이 있는가?
2. 있다면 정답이 S3 Object Lock / S3 Glacier / S3 라이프사이클 인가?
3. 아니라면 시나리오를 다른 주제(CDN/스케일링/DR/DB/서버리스/네트워크 등)로 다시 작성.
...
**시나리오**: 글로벌 SaaS 기업이 프라이빗 서브넷의 Amazon EC2에서 Linux 기반 애플리케이션을 배포하고, 퍼블릭 서브넷의 EC2 인스턴스에서 Linux 기반 배스천 호스트를 함께 가동했습니다. 솔루션 아키텍트는 **사내 네트워크의 인터넷 연결망을 경유하여 배스천 호스트와 애플리케이션 서버 모두에 SSH 터널링으로 보안 접속**을 구성해야 합니다. 모든 EC2 인스턴스의 보안 그룹이 이러한 액세스를 허용하는지 확인해야 합니다.
...
**시나리오**: 실시간 로그 솔루션을 설계 중인 플랫폼 엔지니어링 팀은 초당 수천 개의 이벤트를 수집하고 변환하는 실시간 파이프라인과 자동화 백업 스토리지를 구축하려고 합니다. 개발팀의 친숙도를 고려하여 **최소한의 운영 오버헤드**로 높은 데이터 완전성을 실현하는 것이 핵심 조건입니다.
...
const THEME_GUIDES = {
  ko: {
    ...
    concept_sqs: "**🎯 컨셉 비교: Amazon SQS Standard vs FIFO 큐** — 시나리오 없이 두 큐의 차이(처리량, 순서 보장, 정확히 한 번 전달, 메시지 그룹 ID, 중복 제거)를 직접 묻는 문제 생성. 1문장 질문 형식.",
    concept_sns_sqs: "**🎯 컨셉 비교: Amazon SNS vs SQS** — Pub/Sub vs 메시지 큐, 푸시 vs 풀, fan-out 패턴, 사용 사례 차이를 직접 묻는 문제. 시나리오 없음.",
    concept_storage: "**🎯 컨셉 비교: Amazon EBS vs EFS vs FSx** — 블록 vs 파일, 단일 EC2 vs 다중 마운트, Multi-AZ 지원, 성능 특성, 사용 사례를 비교하는 문제. 시나리오 없음.",
    concept_db: "**🎯 컨셉 비교: Amazon Aurora vs RDS** — 성능, 복제 메커니즘(6 복제본 vs Multi-AZ), 자동 스케일링, MySQL/PostgreSQL 호환성, 비용 차이를 비교하는 문제. 시나리오 없음.",
    concept_dynamodb: "**🎯 컨셉 비교: Amazon DynamoDB 특성** — 파티션 키, 정렬 키, GSI/LSI, DAX, 글로벌 테이블, 온디맨드 vs 프로비저닝 모드 등 DynamoDB 핵심 개념을 직접 묻는 문제. 시나리오 없음.",
    concept_cache: "**🎯 컨셉 비교: ElastiCache Redis vs Memcached** — 영속성, 데이터 구조, 클러스터링, 멀티스레드, 백업/복구 차이를 비교하는 문제. 시나리오 없음.",
    concept_compute: "**🎯 컨셉 비교: Lambda vs ECS Fargate vs EC2** — 서버리스 vs 컨테이너 vs VM, 실행 시간 한계, 콜드 스타트, 비용 모델, 사용 사례를 비교하는 문제. 시나리오 없음.",
    concept_cdn: "**🎯 컨셉 비교: CloudFront vs Global Accelerator** — HTTP/HTTPS 캐싱 vs TCP/UDP 네트워크 가속, Anycast IP, 정적 vs 동적 콘텐츠, 사용 사례를 비교하는 문제. 시나리오 없음.",
    concept_s3class: "**🎯 컨셉 비교: S3 스토리지 클래스 비교** — Standard, Standard-IA, One Zone-IA, Intelligent-Tiering, Glacier Instant Retrieval, Flexible Retrieval, Deep Archive 간 검색 시간/비용/내구성 차이를 비교하는 문제. 시나리오 없음.",
    concept_network: "**🎯 컨셉 비교: Direct Connect vs VPN vs Transit Gateway** — 전용선 vs IPsec vs 허브, 대역폭/지연/암호화 특성, 비용, 사용 사례를 비교하는 문제. 시나리오 없음.",
    concept_secg_nacl: "**🎯 컨셉 비교: Security Group vs Network ACL** — Stateful vs Stateless, 인스턴스 vs 서브넷 레벨, 허용/거부 규칙 차이를 직접 묻는 문제. 시나리오 없음.",
    concept_iam: "**🎯 컨셉 비교: IAM Role vs IAM User vs IAM Policy** — STS AssumeRole, 임시 자격증명 vs 영구 자격증명, 정책 유형(관리형/인라인/리소스 기반) 차이를 직접 묻는 문제. 시나리오 없음.",
    concept_monitor: "**🎯 컨셉 비교: CloudWatch vs CloudTrail vs AWS Config** — 메트릭/로그/알람 vs API 감사 vs 리소스 구성 변경 추적, 사용 사례를 비교하는 문제. 시나리오 없음.",
    concept_kinesis: "**🎯 컨셉 비교: Kinesis Data Streams vs Data Firehose vs MSK** — 실시간 vs 배치, 샤드 vs 토픽, 보존 기간, 컨슈머 모델, Lambda 통합, Kafka 호환성 차이를 비교하는 문제. 시나리오 없음.",
  },
  en: {
    ...
    concept_sqs: "**🎯 Concept: Amazon SQS Standard vs FIFO** — Compare throughput, order guarantee, exactly-once delivery, message group ID, deduplication. NO scenario. 1-sentence question.",
    concept_sns_sqs: "**🎯 Concept: Amazon SNS vs SQS** — Pub/Sub vs message queue, push vs pull, fan-out pattern, use cases. NO scenario.",
    concept_storage: "**🎯 Concept: EBS vs EFS vs FSx** — Block vs file, single EC2 vs multi-mount, Multi-AZ, performance, use cases. NO scenario.",
    concept_db: "**🎯 Concept: Aurora vs RDS** — Performance, replication (6 replicas vs Multi-AZ), auto-scaling, MySQL/PostgreSQL compat, cost. NO scenario.",
    concept_dynamodb: "**🎯 Concept: DynamoDB features** — partition/sort key, GSI/LSI, DAX, global tables, on-demand vs provisioned. NO scenario.",
    concept_cache: "**🎯 Concept: ElastiCache Redis vs Memcached** — Persistence, data structures, clustering, multithreading, backup/restore. NO scenario.",
    concept_compute: "**🎯 Concept: Lambda vs ECS Fargate vs EC2** — Serverless vs container vs VM, runtime limits, cold start, cost model, use cases. NO scenario.",
    concept_cdn: "**🎯 Concept: CloudFront vs Global Accelerator** — HTTP/HTTPS caching vs TCP/UDP network acceleration, Anycast IP, static vs dynamic. NO scenario.",
    concept_s3class: "**🎯 Concept: S3 Storage Classes** — Standard, Standard-IA, One Zone-IA, Intelligent-Tiering, Glacier IR/FR/Deep Archive — retrieval/cost/durability. NO scenario.",
    concept_network: "**🎯 Concept: Direct Connect vs VPN vs Transit Gateway** — Dedicated vs IPsec vs hub, bandwidth/latency/encryption, cost. NO scenario.",
    concept_secg_nacl: "**🎯 Concept: Security Group vs Network ACL** — Stateful vs Stateless, instance vs subnet level, allow/deny rules. NO scenario.",
    concept_iam: "**🎯 Concept: IAM Role vs User vs Policy** — STS AssumeRole, temporary vs long-term creds, managed/inline/resource policies. NO scenario.",
    concept_monitor: "**🎯 Concept: CloudWatch vs CloudTrail vs Config** — Metrics/logs/alarms vs API audit vs resource config tracking. NO scenario.",
    concept_kinesis: "**🎯 Concept: Kinesis Data Streams vs Firehose vs MSK** — Real-time vs batch, shards vs topics, retention, consumer model, Kafka compat. NO scenario.",
  },
  ja: {
    ...
    concept_sqs: "**🎯 概念比較: SQS Standard vs FIFO** — スループット、順序保証、Exactly-once、メッセージグループID、重複排除。シナリオなし。",
    concept_sns_sqs: "**🎯 概念比較: SNS vs SQS** — Pub/Sub vs メッセージキュー、Push vs Pull、Fan-outパターン。シナリオなし。",
    concept_storage: "**🎯 概念比較: EBS vs EFS vs FSx** — ブロック vs ファイル、単一EC2 vs 複数マウント、Multi-AZ、パフォーマンス。シナリオなし。",
    concept_db: "**🎯 概念比較: Aurora vs RDS** — パフォーマンス、レプリケーション、オートスケーリング、互換性、コスト。シナリオなし。",
    concept_dynamodb: "**🎯 概念比較: DynamoDB機能** — パーティション/ソートキー、GSI/LSI、DAX、グローバルテーブル、オンデマンド vs プロビジョニング。シナリオなし。",
    concept_cache: "**🎯 概念比較: ElastiCache Redis vs Memcached** — 永続性、データ構造、クラスタリング、マルチスレッド、バックアップ。シナリオなし。",
    concept_compute: "**🎯 概念比較: Lambda vs ECS Fargate vs EC2** — サーバーレス vs コンテナ vs VM、実行時間制限、コールドスタート、コストモデル。シナリオなし。",
    concept_cdn: "**🎯 概念比較: CloudFront vs Global Accelerator** — HTTP/HTTPSキャッシング vs TCP/UDPネットワーク加速、Anycast IP、静的 vs 動的。シナリオなし。",
    concept_s3class: "**🎯 概念比較: S3ストレージクラス** — Standard、IA、Intelligent-Tiering、Glacier IR/FR/Deep Archive — 取得時間/コスト/耐久性。シナリオなし。",
    concept_network: "**🎯 概念比較: Direct Connect vs VPN vs Transit Gateway** — 専用線 vs IPsec vs ハブ、帯域幅/遅延/暗号化、コスト。シナリオなし。",
    concept_secg_nacl: "**🎯 概念比較: Security Group vs Network ACL** — Stateful vs Stateless、インスタンス vs サブネットレベル、許可/拒否ルール。シナリオなし。",
    concept_iam: "**🎯 概念比較: IAM Role vs User vs Policy** — STS AssumeRole、一時 vs 長期認証情報、管理/インライン/リソースポリシー。シナリオなし。",
    concept_monitor: "**🎯 概念比較: CloudWatch vs CloudTrail vs Config** — メトリクス/ログ/アラーム vs API監査 vs リソース構成追跡。シナリオなし。",
    concept_kinesis: "**🎯 概念比較: Kinesis Data Streams vs Firehose vs MSK** — リアルタイム vs バッチ、シャード vs トピック、保持期間、Kafka互換性。シナリオなし。",
  },
};
